...
// Local
pub use wdao_traits::constants_types::Amount;
pub use wdao_traits::constants_types::CurrencyId;

pub mod precompiles;
use precompiles::WaldoDaoPrecompiles;
...
/// Precompile type.
pub type Precompiles = WaldoDaoPrecompiles<Runtime>;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	...
	impl_name: create_runtime_str!("waldodao-appchain"),
	authoring_version: 1,
...
parameter_types! {
	...
	pub PrecompilesValue: WaldoDaoPrecompiles<Runtime> = WaldoDaoPrecompiles::<_>::new();
}
...
impl pallet_evm::Config for Runtime {
	...
	type PrecompilesType = WaldoDaoPrecompiles<Self>;
...
wdao_orml_traits::parameter_type_with_key! {
	pub ExistentialDeposits: |currency_id: CurrencyId| -> Balance {
		if currency_id == &wdao_traits::constants_types::NATIVE_CURRENCY_ID {
...
impl wdao_orml_tokens::Config for Runtime {
	type Event = Event;
	type Balance = Balance;
	type Amount = Amount;
...
parameter_types! {
	pub const GetNativeCurrencyId: CurrencyId = wdao_traits::constants_types::NATIVE_CURRENCY_ID;
}

pub type AdaptedBasicCurrency = wdao_orml_currencies::BasicCurrencyAdapter<
	Runtime,
	Balances,
	Amount,
	wdao_traits::constants_types::Moment,
>;

impl wdao_orml_currencies::Config for Runtime {
	type Event = Event;
...
impl wdao_orml_nft::Config for Runtime {
	type ClassId = wdao_traits::ClassId;
	type TokenId = wdao_traits::TokenId;
	type ClassData = wdao_traits::ClassData<BlockNumber>;
	type TokenData = wdao_traits::TokenData<AccountId, BlockNumber>;
}

impl wdao_config::Config for Runtime {
	type Event = Event;
}
...
parameter_types! {
	...
	pub const NftModuleId: PalletId = PalletId(*b"wdaonft*");
}

impl wdao_nft::Config for Runtime {
	type Event = Event;
	type ExtraConfig = WdaoConf;
...
impl wdao_order::Config for Runtime {
	type Event = Event;
	type MultiCurrency = Currencies;
	type Currency = Balances;
	type ClassId = wdao_traits::ClassId;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Currencies: wdao_orml_currencies::{Call, Event<T>, Pallet},
		Tokens: wdao_orml_tokens::{Config<T>, Event<T>, Pallet, Storage},
		OrmlNFT: wdao_orml_nft::{Config<T>, Pallet, Storage},
		WdaoConf: wdao_config::{Call, Config<T>, Event<T>, Pallet, Storage},
		WdaoNft: wdao_nft::{Call, Event<T>, Config<T>, Pallet, Storage},
		WdaoOrder: wdao_order::{Call, Config<T>, Event<T>, Pallet, Storage},
	}
);
...
impl_runtime_apis! {
	...
	impl wdao_rpc_runtime_api::WdaoApi<Block> for Runtime {
		fn mint_token_deposit(metadata_len: u32) -> Balance {
			WdaoNft::mint_token_deposit(metadata_len)
		}
		fn add_class_admin_deposit(admin_count: u32) -> Balance {
			WdaoNft::add_class_admin_deposit(admin_count)
		}
		fn create_class_deposit(metadata_len: u32, name_len: u32, description_len: u32) -> (Balance, Balance) {
			WdaoNft::create_class_deposit(metadata_len, name_len, description_len)
		}
	}
