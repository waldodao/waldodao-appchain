...
pub use wdao_orml_traits::nft::{AccountToken, ClassInfo, TokenInfo};
...
pub trait WdaoConfig<AccountId, BlockNumber> {
...
pub trait WdaoNft<AccountId, ClassId, TokenId> {
...
pub struct CategoryData {
	...
	/// The number of orders in this category.
	#[codec(compact)]
	pub count: Balance,
}
...
where
	NFT: WdaoNft<AccountId, ClassId, TokenId>,
	ClassId: Copy,
	TokenId: Copy,
	AccountId: Decode,
{
...
where
	MultiCurrency:
		wdao_orml_traits::MultiCurrency<AccountId, CurrencyId = CurrencyId, Balance = Balance>,
	NFT: WdaoNft<AccountId, ClassId, TokenId>,
	ClassId: Copy,
	TokenId: Copy,
	CurrencyId: Copy,
...
macro_rules! nft_dbg {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "wdao", log::Level::Debug, $($msg),+);
	};
}
...
macro_rules! nft_info {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "wdao", log::Level::Info, $($msg),+);
	};
}
...
macro_rules! nft_err {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "wdao", log::Level::Error, $($msg),+);
	};
}
