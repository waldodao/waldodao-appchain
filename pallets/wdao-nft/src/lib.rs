...
use sp_std::vec::Vec;
use wdao_orml_traits::{MultiCurrency, MultiReservableCurrency};

pub use pallet::*;
use wdao_orml_nft::{ClassInfoOf, TokenInfoOf};
use wdao_traits::*;

pub type TokenIdOf<T> = <T as wdao_orml_nft::Config>::TokenId;
pub type ClassIdOf<T> = <T as wdao_orml_nft::Config>::ClassId;
...
pub mod migrations {
	use super::*;
	pub type OldClass<T> = wdao_orml_nft::ClassInfo<
		TokenIdOf<T>,
		<T as frame_system::Config>::AccountId,
		OldClassData,
	>;
	pub type NewClass<T> = wdao_orml_nft::ClassInfo<
		TokenIdOf<T>,
		<T as frame_system::Config>::AccountId,
		ClassData<BlockNumberOf<T>>,
	>;
	pub type OldToken<T> = wdao_orml_nft::TokenInfo<TokenIdOf<T>, OldTokenData>;
	pub type NewToken<T> = wdao_orml_nft::TokenInfo<
		TokenIdOf<T>,
	...
	pub fn do_migrate<T: Config>() -> Weight {
		// migrate classes
		wdao_orml_nft::Classes::<T>::translate::<OldClass<T>, _>(|_, p: OldClass<T>| {
		...
		// migrate tokens
		wdao_orml_nft::Tokens::<T>::translate::<OldToken<T>, _>(|_, _, p: OldToken<T>| {
...
pub mod pallet {
		...
		> + pallet_proxy::Config
	{
		...
		type ExtraConfig: WdaoConfig<Self::AccountId, BlockNumberFor<Self>>;
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			} in &self.classes
			{
				...
				assert!(wdao_orml_nft::Classes::<T>::get(*class_id).is_none(), "Dup class id");
				wdao_orml_nft::NextClassId::<T>::set(*class_id);
				...
				wdao_orml_nft::Pallet::<T>::create_class(&owner, class_metadata.clone(), data)
					.unwrap();
				...
				{
					assert!(
						wdao_orml_nft::Tokens::<T>::get(*class_id, *token_id).is_none(),
						"Dup token id"
					);
					...
					wdao_orml_nft::NextTokenId::<T>::insert(*class_id, *token_id);
					wdao_orml_nft::Pallet::<T>::mint(
						token_owner,
						*class_id,
				...
				wdao_orml_nft::NextTokenId::<T>::insert(
					*class_id,
					max_token_id.checked_add(&One::one()).unwrap(),
				);
			}
			wdao_orml_nft::NextClassId::<T>::set(max_class_id.checked_add(&One::one()).unwrap());
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn update_token_royalty(
			...
			let who = ensure_signed(origin)?;
			wdao_orml_nft::Tokens::<T>::try_mutate(
				class_id,
				token_id,
				|maybe_token| -> DispatchResultWithPostInfo {
					let token_info: &mut TokenInfoOf<T> =
						...
						wdao_orml_nft::Pallet::<T>::total_count(&who, (class_id, token_id))
							== token_info.quantity,
					...
					token_info.data.royalty_rate = charge_royalty
						.ok_or_else(|| -> Result<PerU16, DispatchError> {
							let class_info: ClassInfoOf<T> =
								wdao_orml_nft::Pallet::<T>::classes(class_id)
		...
		pub fn mint(
			...
			let to = T::Lookup::lookup(to)?;
			let class_info: ClassInfoOf<T> =
				wdao_orml_nft::Pallet::<T>::classes(class_id).ok_or(Error::<T>::ClassIdNotFound)?;
			let _ = Self::do_mint(
				&who,
				&to,
				&class_info,
		...
		pub fn burn(
			...
			if let Some(token_info) =
				wdao_orml_nft::Pallet::<T>::burn(&who, (class_id, token_id), quantity)?
			{
		...
		pub fn destroy_class(
			...
			wdao_orml_nft::Pallet::<T>::destroy_class(&who, class_id)?;
...
impl<T: Config> Pallet<T> {
	...
	fn do_mint(
		...
		let token_id: TokenIdOf<T> =
			wdao_orml_nft::Pallet::<T>::mint(to, class_id, metadata, data, quantity)?;
	...
	pub fn do_create_class(
		...
		let next_id = wdao_orml_nft::Pallet::<T>::next_class_id();
		...
		wdao_orml_nft::Pallet::<T>::create_class(&owner, metadata, data)?;
	...
	pub fn do_transfer(
		...
		wdao_orml_nft::Pallet::<T>::transfer(from, to, (class_id, token_id), quantity)?;
	...
	pub fn contract_tokens(
		...
		wdao_traits::ContractTokenInfo<NFTMetadata, Quantity, Balance, BlockNumber, T::AccountId>,
	> {
		wdao_orml_nft::Pallet::<T>::tokens(class_id, token_id).map(|t: TokenInfoOf<T>| {
			wdao_traits::ContractTokenInfo {
				metadata: t.metadata,
				quantity: t.quantity.saturated_into(),
				data: wdao_traits::ContractTokenData {
	...
	pub fn tokens_by_owner(
		...
		wdao_orml_nft::TokensByOwner::<T>::iter_prefix(account_id)
			.map(|((c, t), a)| (c, t, a))
	...
	pub fn owners_by_token(
		...
		wdao_orml_nft::OwnersByToken::<T>::iter_prefix((class_id, token_id))
			.map(|(account_id, _): (T::AccountId, ())| account_id)
			.skip(page.saturating_mul(page_size) as usize)
			.take(page_size as usize)
			.map(|account_id| {
				let at = wdao_orml_nft::TokensByOwner::<T>::get(&account_id, (class_id, token_id))
					.unwrap_or_default();
				(account_id, at)
			})
			.collect()
...
impl<T: Config> wdao_traits::WdaoNft<T::AccountId, ClassIdOf<T>, TokenIdOf<T>> for Pallet<T> {
	fn peek_next_class_id() -> ClassIdOf<T> {
		wdao_orml_nft::NextClassId::<T>::get()
	}
	...
	fn account_token(
		...
		wdao_orml_nft::Pallet::<T>::tokens_by_owner(who, (class_id, token_id)).unwrap_or_default()
	}
	...
	fn reserve_tokens(
		...
		wdao_orml_nft::Pallet::<T>::reserve(who, (class_id, token_id), quantity)
	}
	...
	fn unreserve_tokens(
		...
		wdao_orml_nft::Pallet::<T>::unreserve(who, (class_id, token_id), quantity)
	}
	...
	fn token_charged_royalty(
		...
		let token: TokenInfoOf<T> = wdao_orml_nft::Tokens::<T>::get(class_id, token_id)
