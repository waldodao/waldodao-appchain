...
use sp_std::vec::Vec;
use wdao_traits::{BitFlags, ClassId, ClassProperty, GlobalId, Properties, Signature, TokenId};

pub struct WdaoExtension<Runtime>(sp_std::marker::PhantomData<Runtime>);
...
impl<Runtime> ChainExtension<Runtime> for WdaoExtension<Runtime>
where
	...
	Runtime: wdao_nft::Config<ClassId = ClassId, TokenId = TokenId>,
{
		...
		<E::T as SysConfig>::AccountId: UncheckedFrom<<E::T as SysConfig>::Hash> + AsRef<[u8]>,
	{
		match func_id {
			...
			2002 => {
				...
				let (owner, class_id) = wdao_nft::Pallet::<Runtime>::do_create_class(
					&caller,
					metadata,
					name,
					description,
			...
			2003 => {
				...
				let (class_owner, beneficiary, class_id, token_id, quantity) =
					wdao_nft::Pallet::<Runtime>::do_proxy_mint(
						&caller,
						&to,
						class_id,
						metadata,
			...
			2004 => {
				...
				wdao_nft::Pallet::<Runtime>::do_transfer(
			...
			1001 => {
				...
				let r = wdao_nft::Pallet::<Runtime>::contract_tokens(class_id, token_id);
