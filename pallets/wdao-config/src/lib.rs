...
use wdao_traits::{
	...
	time, CategoryData, NFTMetadata, WdaoConfig,
};
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			for a in &self.white_list {
				<Pallet<T> as WdaoConfig<T::AccountId, T::BlockNumber>>::do_add_whitelist(a);
			}
			for c in &self.category_list {
				<Pallet<T> as WdaoConfig<T::AccountId, T::BlockNumber>>::do_create_category(
					c.clone(),
				)
				.unwrap();
			}
...
impl<T: Config> WdaoConfig<T::AccountId, BlockNumberFor<T>> for Pallet<T> {
	...
	fn do_create_category(metadata: NFTMetadata) -> DispatchResultWithPostInfo {
		let category_id = <Self as WdaoConfig<T::AccountId, BlockNumberFor<T>>>::get_then_inc_id()?;
