...
use waldodao_appchain_runtime::{
	currency::{OCTS, UNITS as WALDO},
	...
	TechComitteeCollectiveConfig, TokensConfig, WdaoConfConfig, WdaoNftConfig,
...
pub fn staging_testnet_config() -> Result<ChainSpec, String> {
	...
	let properties = get_properties("WALDO", 18, 42);

	Ok(ChainSpec::from_genesis(
		// Name
		"WaldoDao Staging Testnet",
		// ID
		"waldodao_staging_testnet",
		ChainType::Live,
		move || {
			genesis(
				// WASM Binary
...
pub fn development_testnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		// Name
		"WaldoDao Development Testnet",
		// ID
		"waldodao_development_testnet",
		ChainType::Live,
		move || {
			genesis(
				// WASM Binary
		...
		None,
		// Protocol ID
		Some("waldodao-development-testnet"),
		// Fork ID
		None,
		// Properties
		Some(properties),
...
pub fn development_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		// Name
		"WaldoDao Development",
		// ID
		"waldodao_development",
...
fn genesis(
	...
	GenesisConfig {
		...
		wdao_nft: WdaoNftConfig {
			classes: vec![
				// Waldo Dao Default Collection
				wdao_traits::ClassConfig {
					class_id: 0,
					...
					.unwrap(),
					category_ids: vec![0],
					name: String::from_utf8(b"Waldo Dao".to_vec()).unwrap(),
					description: String::from_utf8(b"Waldo Dao Collection".to_vec()).unwrap(),
					properties: 1 | 2,  // 1 = Transferable, 2 = Burnable
					royalty_rate: PerU16::from_percent(0),
					admins: vec![
						AccountId::from_ss58check(
							"5F1m9ZZ6a4Jitet2AriPE6P3SchwZZwXKkQuGh8RrBxwzgPs",
						)
						.unwrap(),
					],
					tokens: vec![
						wdao_traits::TokenConfig {
							token_id: 0,
							token_metadata: String::from_utf8(
								br#"{\"name\":\"Waldo Dao\",\"description\":\"Waldo DAO bridge Talent and Hirer.\",\"image\":\""}"#.to_vec(),
							)
							.unwrap(),
