...
use waldodao_appchain_runtime::{opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		waldodao_appchain_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		waldodao_appchain_runtime::native_version()
	}
}
...
pub fn frontier_database_dir(config: &Configuration) -> std::path::PathBuf {
	let config_dir = config
		...
		.unwrap_or_else(|| {
			BasePath::from_project("", "", "waldodao").config_dir(config.chain_spec.id())
		});
